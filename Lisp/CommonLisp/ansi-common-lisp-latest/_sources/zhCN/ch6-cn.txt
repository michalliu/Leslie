...
关键字与其相关的参数可以被剩馀参数收集起来，并传递给其他预期收到这些参数的函数。举例来说，我们可以这样定义 ``adjoin`` ：
...
函数 ``curry`` 与 ``rcurry`` （“right curry”）精神上与前一小节的 ``make-adder`` 相同。两者皆接受一个函数及某些参数，并返回一个预期剩馀参数的新函数。下列任一个函数等同於 ``(make-adder 3)`` :
