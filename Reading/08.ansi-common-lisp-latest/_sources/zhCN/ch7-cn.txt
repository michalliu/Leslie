...
在图 7.1 的代码，实现了环状缓冲区的操作。 ``buf`` 有五个字段 (field): 一个包含存入缓冲区的向量，四个其它字段用来放指向向量的索引 (indices)。两个索引是 ``start`` 与 ``end`` ，任何环状缓冲区的使用都会需要这两个索引: ``start`` 指向缓冲区的第一个值，当我们取出一个值时， ``start`` 会递增 (incremented)； ``end`` 指向缓冲区的最後一个值，当我们插入一个新值时， ``end`` 会递增。
